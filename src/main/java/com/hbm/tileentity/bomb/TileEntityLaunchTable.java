...
public class TileEntityLaunchTable extends TileEntityLoadedBase implements ITickable, IEnergyUser, IFluidHandler, ITankPacketAcceptor, SimpleComponent, IGUIProvider {
		...
		if (!world.isRemote) {
			...
			for(int x = -4; x <= 4; x++) {
				for(int z = -4; z <= 4; z++) {
					...
					if(world.getRedstonePowerFromNeighbors(pos.add(x, 0, z)) > 0 && canLaunch()) {
						launch();
						break outer;
					}
				}
