...
public class ItemMetaHandler implements IFixableData {
    ...
    public @NotNull NBTTagCompound fixTagCompound(NBTTagCompound tag) {
        if (tag.hasKey("id", 8)) {
            ...
            if (id.startsWith("hbm:wire_") || id.startsWith("hbm:bolt_")) {
                ...
                if (matName.equals("Red_copper")) matName = "Mingrade";
                if (matName.equals("Advanced_alloy")) matName = "AdvancedAlloy";
                if (matName.equals("Magnetized_tungsten")) matName = "MagnetizedTungsten";
                if (matName.equals("Dura_steel")) matName = "DuraSteel";
                try {
            ...
            } else if (id.startsWith("hbm:ore_bedrock")) {
                int ore = tag.getShort("Damage");
                int newMeta = ore;
                if (ore > 15) {
                    newMeta += 2;
                }
